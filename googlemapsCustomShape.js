...
gmapsCustomShapes.Polygon.Shape = function (point, r1, r2, r3, r4, rotation, vertexCount, strokeColour, strokeWeight, Strokepacity, fillColour, fillOpacity, opts, tilt) {
    ...
    if (point.distanceFrom) {
        ...
        latConv = calcDistance(point, new gmaps.LatLng(point.lat() + 0.1, point.lng())) * 10;
        lngConv = calcDistance(point, new gmaps.LatLng(point.lat(), point.lng() + 0.1)) * 10;
    }
...
function EOffset(point, easting, northing) {
    var latConv = point.distanceFrom(new gmaps.LatLng(point.lat() + 0.1, point.lng())) * 10;
    var lngConv = point.distanceFrom(new gmaps.LatLng(point.lat(), point.lng() + 0.1)) * 10;
    return new gmaps.LatLng(point.lat() + northing / latConv, point.lng() + easting / lngConv)
}
...
function EOffsetBearing(point, dist, bearing) {
    ...
    return new gmaps.LatLng(point.lat() + lat, point.lng() + lng)
}
...
//gmaps.Polyline.Shape = function (point, r1, r2, r3, r4, rotation, vertexCount, colour, weight, opacity, opts, tilt) {
//    var rot = -rotation * Math.PI / 180;
//    var points = [];
...
//        points.push(new gmaps.LatLng(point.lat() + lat, point.lng() + lng));
//    }
//    return (new gmaps.Polyline(points, colour, weight, opacity, opts))
//}

//gmaps.Polyline.Circle = function (point, radius, colour, weight, opacity, opts) {
//    return gmaps.Polyline.Shape(point, radius, radius, radius, radius, 0, 100, colour, weight, opacity, opts)
//}

//gmaps.Polyline.RegularPoly = function (point, radius, vertexCount, rotation, colour, weight, opacity, opts) {
//    rotation = rotation || 0;
//    var tilt = !(vertexCount & 1);
//    return gmaps.Polyline.Shape(point, radius, radius, radius, radius, rotation, vertexCount, colour, weight, opacity, opts, tilt)
//}

//gmaps.Polyline.Star = function (point, r1, r2, points, rotation, colour, weight, opacity, opts) {
//    rotation = rotation || 0;
//    return gmaps.Polyline.Shape(point, r1, r1, r2, r2, rotation, points * 2, colour, weight, opacity, opts)
//}

//gmaps.Polyline.Ellipse = function (point, r1, r2, rotation, colour, weight, opacity, opts) {
//    rotation = rotation || 0;
//    //return gmaps.Polyline.Shape(point, r1, r2, r1, r2, rotation, 100, colour, weight, opacity, opts)
